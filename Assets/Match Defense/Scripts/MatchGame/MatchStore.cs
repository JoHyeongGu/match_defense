...
namespace MatchDefense.MatchGame
{
    public class MatchStore : MonoBehaviour
    {
        ...
        [SerializeField] private float storeBottomPosition = 400f;
        ...
        [Header("<color=yellow>Unit</color>")]
        [SerializeField] private LayerMask unitLayer;
        [SerializeField] private float checkRadius = 0.5f;
        [SerializeField] private float dummyHeight = 0.8f;
        ...
        public float GetStoreBottomWorldY()
        {
            ...
            if (
                ...
                && panelRenderer.panelSettings.scaleMode == PanelScaleMode.ScaleWithScreenSize
            )
            {
                Vector2 refRes = panelRenderer.panelSettings.referenceResolution;
                float match = panelRenderer.panelSettings.match;
            ...
            if (Physics.Raycast(ray, out RaycastHit hit, 1000f, groundLayer))
            {
                if (hit.collider.CompareTag(groundTag))
                {
                    ...
                    else
                    {
                        ...
                        isValidSpawnPosition = false;
                        dragDummy.GetComponent<SpriteRenderer>().color = new Color(
                            1f,
                            0f,
                            0f,
                            0.5f
        ...
        private void OnDrawGizmos()
        {
            ...
            {
                if (panelRenderer.panelSettings.scaleMode == PanelScaleMode.ScaleWithScreenSize)
                {
                    Vector2 refRes = panelRenderer.panelSettings.referenceResolution;
                    float match = panelRenderer.panelSettings.match;
                    float scaleX = camWidth / refRes.x;
            ...
            if (count > 0)
            {
                ...
                float startX =
                    uiCamera.transform.position.x - (totalW / 2f) + (wWidth / 2f) + wMargin;
            ...
            Vector3 lineEnd = uiCamera.ScreenToWorldPoint(
                new Vector3(camWidth, scaledBottom, zDepth)
            );
